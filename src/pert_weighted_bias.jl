...
function main()
    ...
    for j in target_biases
        ...
        open(joinpath(target_dir, "weighted_change.csv"), "w") do out_io
            ...
            for vector_filename in readdir(embedding_dir)
                if startswith(vector_filename, "vectors-$target")
                    ...
                    propB̃ = Bias.weighted_effect_size(W, vocab, ivocab, weat_idx_sets[i], family=:prop)
                    logB̃ = Bias.weighted_effect_size(W, vocab, ivocab, weat_idx_sets[i], family=:log)
                    polyB̃ = Bias.weighted_effect_size(W, vocab, ivocab, weat_idx_sets[i], family=:poly)
                    data = [vector_filename, "baseline", 0, 1, finfo.seed, trueB̃, propB̃, logB̃, polyB̃]
            ...
            for vector_filename in readdir(target_dir)
                if startswith(vector_filename, "vectors-")
                    ...
                    logB̃ = Bias.weighted_effect_size(W, vocab, ivocab, weat_idx_sets[i], family=:log)
                    polyB̃ = Bias.weighted_effect_size(W, vocab, ivocab, weat_idx_sets[i], family=:poly)
                    data = [vector_filename, fields..., trueB̃, propB̃, logB̃, polyB̃]
                    println(out_io, join(data, ","))
                end
            end
        end
